...
func TestArtifact_Query(t *testing.T) {
	...
	// 9. Document artifact (review-resolution)
	{
		...
		if len(a.Visibility) != 6 || a.Visibility[0] != knowledge.RolePlanner || a.Visibility[1] != knowledge.RoleBuilder || a.Visibility[2] != knowledge.RoleReviewer || a.Visibility[3] != knowledge.RoleNavigator || a.Visibility[4] != knowledge.RoleCartographer || a.Visibility[5] != knowledge.RoleVerifier {
			t.Errorf("expected visibility [planner builder reviewer navigator cartographer verifier], got %v", a.Visibility)
		}
