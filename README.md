...
2. **Taste Gate & Complexity Budget (`cartography-taste-gate-advisory`)**:
   Before rendering, Cartographer exercises a **Taste Gate** assessment to evaluate visual suitability against strict complexity budgets (≤12 nodes for structural diagrams, ≤12 transitions for sequence flows). When markdown tables or structured prose provide superior clarity, Cartographer issues an advisory pushback (`ADVISORY_ISSUED`) directly to Step 6 without rendering markup.
